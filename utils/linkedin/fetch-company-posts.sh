...
# LinkedIn Company Posts Fetcher via RapidAPI
# Usage: ./fetch-company-posts.sh <company-username> [start]
# Example: ./fetch-company-posts.sh assorthealth
# Example: ./fetch-company-posts.sh fleetworksai 50  # Page 2
...
USERNAME="${1:?Usage: $0 <company-username> [start]}"
START="${2:-0}"

# Create output directory (companies, not profiles)
OUTPUT_DIR="$PROJECT_ROOT/docs/linkedin/companies/$USERNAME"
mkdir -p "$OUTPUT_DIR"
OUTPUT="$OUTPUT_DIR/posts-$(date +%Y-%m-%d).json"

echo "Fetching company posts for: $USERNAME (start=$START)"

# Build URL with optional pagination
URL="https://real-time-data-enrichment.p.rapidapi.com/get-company-posts?username=$USERNAME"
if [[ "$START" != "0" ]]; then
  URL="$URL&start=$START"
fi
...
case "$HTTP_CODE" in
  200)
    ...
    if [[ "$PAGINATION_TOKEN" != "none" && "$PAGINATION_TOKEN" != "null" ]]; then
      ...
      echo "  ./fetch-company-posts.sh $USERNAME $((START + 50))"
    fi

    # Show recent post titles
    echo ""
    echo "Recent company posts:"
    echo "$BODY" | jq -r '.data[:5][] | "- " + (.text // "No text")[0:80] + "..."' 2>/dev/null || echo "  (check JSON for structure)"

    # Record scrape time
    echo "$(date -u +%Y-%m-%dT%H:%M:%SZ)" > "$OUTPUT_DIR/scraped-at.txt"
    ;;
  429)
